...
<head>
    ...
    <title>{{$p_info->condo_address }}</title>
...
<body>
    ...
    <body>
        <div class="container mt-5">
            <h3>Property ID: {{$p_info->condo_id}}</h3>
            <form id="deleteForm" action="{{ route('admin.deletePropertyCondo', ['id' => $p_info->condo_id]) }}"
                method="POST">
                @csrf
                @method('DELETE')
            ...
            <form class="mt-5" method="POST"
                action="{{ route('admin.updatePropertyCondo', ['id' => $p_info->condo_id]) }}">
                @csrf
                @method('PUT')
            ...
            <form action="{{ route('admin.addPropertyImageCondo', ['id' => $p_info->condo_id]) }}" method="POST"
                ...
                <div class="col-md-3">
                    <div class="card mb-3">
                        ...
                        <div class="card-body text-center">
                            <form
                                action="{{ route('admin.deletePropertyImageCondo', ['propertyId' => $p_info->condo_id, 'imageId' => $image['id']]) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
